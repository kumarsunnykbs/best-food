		...
		if ( $this->scenario=="refund"){	
			...
			if($auto_approval->meta_value==1){*/
				...
				if($model->save()){
				   ...
				   ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/processrefund?".http_build_query($get_params) );
				}
			//}
			...
			if($model->save()){
			   ...
			   ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/sendinvoice?".http_build_query($get_params) );
			}
