...
{		
	...
	public function actionupdateOrderSummary()
	{
		try {
		   ...
		   $order_uuid = isset($this->data['order_uuid'])?$this->data['order_uuid']:'';					   		   
		   $order_uuid = '18cf4e83-4b32-11ec-a3e1-9c5c8e164c2c';
...
HTML;
	...
	{		
		try {
			...
			$accepting_order = isset($data['accepting_order'])?$data['accepting_order']['meta_value']:false;
			$accepting_order = $accepting_order==1?true:false;
			$pause_time = isset($data['pause_time'])?trim($data['pause_time']['meta_value']):'';			
			
			if(!$accepting_order){							
				$pause_time = Date_Formatter::dateTime($pause_time,"yyyy-MM-ddTHH:mm");
			} else $pause_time = Date_Formatter::dateTime(date("c"),"yyyy-MM-ddTHH:mm");
		    ...
		    if($model){
		       foreach ($model as $item) {		       	  
		       	  ...
		       	  $data[] = array(
		       	    ...
		       	    'category_name'=>Yii::app()->input->xssClean($item->category_name),
