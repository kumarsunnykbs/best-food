...
class COrders
{
    ...
    public static function orderHistory($client_id=0,$q='',$page=0,$limit=10)
    {
    ...
    public static function orderHistoryTotal($client_id=0)
    {
    	$stmt="
    ...
    public static function getOrderHistory($client_id=0,$q='',$page=0,$limit=10,$lang=KMRS_DEFAULT_LANGUAGE)
    {    	    	    	
    	$data = COrders::orderHistory($client_id,$q,$page,$limit,$q);        	
    	...
    	if($status_delivered = AR_admin_meta::getValue('status_delivered')){
    		$status_delivered = array($status_delivered['meta_value']);
    	}
    	
    	$status_allowed_cancelled = COrders::getStatusAllowedToCancel();
    	$status_allowed_review = $status_delivered;
    ...
    {    	
		...
		if($items->save()){
			...
			if($scenario=="update_cart"){				
				...
				ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/updatesummary?".http_build_query($get_params) );
			}
