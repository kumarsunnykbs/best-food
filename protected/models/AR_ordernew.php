...
{	
	...
	{										
		...
		/*if(IS_FRONTEND){
		   ERunActions::touchUrl( Yii::app()->createAbsoluteUrl("/task/afterpurchase",$get_params) );
		} else {
		   ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/afterpurchase?".http_build_query($get_params) );
		}*/
		...
		if ( $this->scenario=="new_order"){
			...
			$this->insertHistory();						
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/afterupdatestatus?".http_build_query($get_params) );			
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/trackorder?".http_build_query($get_params) );	
			
		} elseif ($this->scenario=="cancel_order" || $this->scenario=="reject_order" ){
			
			$this->insertHistory();						
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/afterordercancel?".http_build_query($get_params) );	
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/trackorder?".http_build_query($get_params) );			
			
		} elseif ($this->scenario=="delay_order"){
			
			$this->insertHistory();						
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/afterdelayorder?".http_build_query($get_params) );
			
		} elseif ($this->scenario=="adjustment"){
			ERunActions::touchUrl( CommonUtility::getHomebaseUrl()."/task/afteradjustment?".http_build_query($get_params) );
